...
infile2 = argv[2]
infile3 = argv[3]
ntuple_filepu = ROOT.TFile(infile1)
ntuple_filecal = ROOT.TFile(infile2)
ntuple_filejec = ROOT.TFile(infile3)

######## LABEL & SAVE WHERE #########

if len(argv)>3:
   saveWhere='~/www/Research/'+argv[4]+'_'
else:
...
jet_ntuple_pu = ntuple_filepu.Get("jetEfficiency/Ntuple")
jet_ntuple_cal = ntuple_filecal.Get("jetEfficiency/Ntuple")
jet_ntuple_jec = ntuple_filejec.Get("corrjetEfficiency/Ntuple")
...
def make_l1gj_efficiency(denom, num):
    ''' Make an efficiency graph with the "UCT" style '''
    eff = ROOT.TGraphAsymmErrors(num, denom)
    eff.SetMarkerStyle(24)
    eff.SetMarkerColor(ROOT.EColor.kPink)
    eff.SetMarkerSize(1.5)
    eff.SetLineColor(ROOT.EColor.kBlack)
    return eff
...
def compare_efficiencies(ntuplep,ntuplec,ntuplej, variable, l1PtCut, binning, filename,
    ...
    l1gc = make_l1g_efficiency(denomc, num_l1c)
    l1gj = make_l1gj_efficiency(denomj, num_l1j)
    l1 = make_l1_efficiency(denomp, num_l1p)
    ...
    legend.AddEntry(l1gj, "UCT2015 PU JEC", "pe")
    legend.AddEntry(l1gc, "UCT2015 PU PION CAL", "pe")
    legend.AddEntry(l1, "UCT2015 PU NO JEC/CAL", "pe")
    legend.Draw()
...
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  100, [40, 0, 300],
                    'jet_pt_trg100',
                     "Jet 100 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  150, [40, 0, 300],
                     'jet_pt_trg150',
                     "Jet 150 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  200, [40, 0, 300],
                     'jet_pt_trg200',
                     "Jet 200 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  30, [40, 0, 200],
                     'jet_pt_trg30',
                     "Jet 30 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  40, [40, 0, 200],
                     'jet_pt_trg40',
                     "Jet 40 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  50, [40, 0, 200],
                     'jet_pt_trg50',
                     "Jet 50 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  60, [40, 0, 200],
                     'jet_pt_trg60',
                     "Jet 60 efficiency", "RECO p_{T} (GeV)",True)
compare_efficiencies(jet_ntuple_pu,jet_ntuple_cal,jet_ntuple_jec, 'recoPt',  70, [40, 0, 200],
                    'jet_pt_trg70',
